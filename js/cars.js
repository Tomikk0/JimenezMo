...
function buildCarDisplayData(car, hasTagMap) {
  ...
  if (car.EladasiAr && !isNaN(car.EladasiAr)) {
    const keszpenzErtek = Math.round(car.EladasiAr * 0.975);
...
function openAddCarModal() {
  try {
    ...
    // Admin tag választó megjelenítése
    updateAddCarAdminSection();
...
function updateAddCarAdminSection() {
  const adminSection = document.getElementById('adminAddedBySection');
  const addedBySelect = document.getElementById('addedBySelect');
  
  if (!adminSection || !addedBySelect) return;

  // Ha admin, megjelenítjük a tag választót
  if (currentUser && currentUser.role === 'admin') {
    adminSection.style.display = 'block';
    
    // Tag lista feltöltése
    addedBySelect.innerHTML = '<option value="">— saját nevemben —</option>';
    
    if (Array.isArray(tagOptions) && tagOptions.length > 0) {
      tagOptions.forEach(tag => {
        const option = document.createElement('option');
        option.value = tag.name;
        option.textContent = tag.name + (tag.rank ? ` (${tag.rank})` : '');
        addedBySelect.appendChild(option);
      });
    }
  } else {
    // Ha nem admin, elrejtjük
...
async function addCar() {
  try {
    ...
    // 🔧 ÚJ: Admin esetén nézd meg, ki van kiválasztva
    let addedByName = currentUser.tagName;
    
    if (currentUser.role === 'admin') {
      const addedBySelect = document.getElementById('addedBySelect');
      if (addedBySelect && addedBySelect.value) {
        addedByName = addedBySelect.value;
        console.log('🔑 Admin másik tag nevében ad hozzá autót:', addedByName);
      }
    }
    ...
    const carData = {
      ...
      added_by: addedByName, // 🔧 Módosított rész
      sold: false,
      image_data_url: imageDataUrl,
      is_gallery: false
    };

    console.log('📦 Autó hozzáadás adatai:', carData);
    ...
    if (error) {
      ...
      // Sikeres üzenet személyre szabva
      const successMsg = currentUser.role === 'admin' && addedByName !== currentUser.tagName
        ? `Autó sikeresen hozzáadva ${addedByName} nevében!`
